...
readonly BUILDROOT_MAKE_OPTS="-C "${BUILDROOT_DIR}" O="${BUILDROOT_BUILD_DIR}""
...
readonly MODULE_MAKE_OPTS="-j"$(nproc)" LLVM=1 KDIR="${LINUX_BUILD_DIR}""
...
function setup_rust_tooling() {
    ...
    local KERNEL_TOOL_VERSION_SCRIPT="${LINUX_DIR}/scripts/min-tool-version.sh"
    ...
    local KERNEL_VERSION="$(make ${LINUX_MAKE_OPTS} -s kernelversion)"
